...
public class RegistryKlientFakeImplTest {
    ...
    @Test
    public void skalReturnereSikkerhetspolicyDeaktivert() throws Exception {
        RegistryKlient registryKlient = new RegistryKlientBuilder("file://oppslagstjeneste.properties").byggRegistryKlient();
        Map<String,String> tjenesteAttributter = registryKlient.hentTjenesteAttributter(STSTjenesteKonstanter.URN_STS_UTSTED_SAML);
        assertThat(tjenesteAttributter.get(STSTjenesteKonstanter.STS_SIKKERHETSPOLICY_TJENESTEATTRIBUTT))
                .isEqualTo(SikkerhetspolicyOppslagstjeneste.DEAKTIVERT.getValue());
    }

    @Test(expected = MagnetApplicationException.class)
    public void skalKasteExceptionHvisFilIkkeFinnes() throws Exception {
        RegistryKlientImpl.med("file://finnesIkke.properties").bygg();
    }

    @Test(expected = MagnetApplicationException.class)
    public void skalGiFeilmeldingOmFeilFiltypeVedFeilFiltype() {
        new RegistryKlientBuilder("file://tull.doc").byggRegistryKlient();
    }

    @Test(expected = MagnetApplicationException.class)
    public void skalGiFeilmeldingOmFeilFiltypeVedIngenFiltype() {
        new RegistryKlientBuilder(URI.create("file://fake")).byggRegistryKlient();
    }
